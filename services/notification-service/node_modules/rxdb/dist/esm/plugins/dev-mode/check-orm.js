 ...
 */
export var RX_ATTACHMENT_RESERVED_NAMES = ['doc', 'id', 'type', 'length', 'digest', 'remove', 'getData', 'getStringData', 'getDataBase64'];

/**
 * checks if the given static methods are allowed
 * @throws if not allowed
 */
export function checkOrmMethods(statics, additionalReservedNames) {
  if (!statics) {
    return;
  }
  Object.entries(statics).forEach(([k, v]) => {
    ...
    if (rxCollectionProperties().includes(k) || rxDocumentProperties().includes(k) || additionalReservedNames && additionalReservedNames.includes(k)) {
