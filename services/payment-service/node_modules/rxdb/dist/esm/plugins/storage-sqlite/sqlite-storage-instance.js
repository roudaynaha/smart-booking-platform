...
import { getPrimaryFieldOfPrimaryKey, categorizeBulkWriteRows, ensureNotFalsy, addRxStorageMultiInstanceSupport, promiseWait, getQueryMatcher, hasPremiumFlag } from "../../index.js";
...
export var RxStorageInstanceSQLite = /*#__PURE__*/function () {
  ...
  _proto.bulkWrite = async function bulkWrite(documentWrites, context) {
    ...
    if (!shownNonPremiumLog && !(await hasPremiumFlag())) {
      console.warn(['-------------- RxDB SQLite Trial Storage ---------------------------------', 'You are using the free *trial* SQLite-based RxStorage implementation: https://rxdb.info/rx-storage-sqlite.html?console=sqlite-trial', 'This storage is intended only for evaluation purposes and comes with strict limitations (no indexes, no attachments, and a ~300-document cap).', 'For production use and optimal performance, we strongly recommend upgrading to the premium SQLite storage.', 'Premium version: https://rxdb.info/premium/?console=sqlite', 'If you already have premium access, you can disable this message by calling setPremiumFlag() from rxdb-premium/plugins/shared.', '----------------------------------------------------------------------------'].join('\n'));
